...
class PPM {
    ...
    constructor() {
        console.log('PPM constructor called');
    }
    ...
    sampler2D(x: number, y: number): [number, number, number] {
        // scale x and y to be in image coordinates 
        x = Math.floor(x * this._width) % this._width;
        y = Math.floor(y * this._height) % this._height;
